...
// essas operações serão chamadas dentro de Startup.cs em configure
namespace SalesWebMVC.Data
{
    public class SeedingService
    {
        //Registrar dependencia com o DB context
        private SalesWebMVCContext _context;

        //construtor gerado com a ferramenta
        public SeedingService(SalesWebMVCContext context)
        {
            _context = context;
        }

        //vai ser responsável por popular o banco de dados
        public void Seed()
        {
            //Any() verifica se tem alguma informação na tabela departament
            if (_context.Department.Any() ||
                _context.Seller.Any() ||
                _context.salesRecords.Any())
            {
                return;  // não retorna nada se o banco já foi populado
            }

            //populando banco / instancia dos objetos
            ...
            //vamos adicionar os objetos no banco - AddRange permite add varios objetos de uma vez
            ...
            //salva e confirma as operações no banco
